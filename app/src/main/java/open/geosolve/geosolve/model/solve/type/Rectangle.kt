                ...
                if ((thisElement.getValue() != null) and (figure.mLines[(i + 2) % 4].getValue() == null))
                    figure.mLines[(i + 2) % 4].setDependentValueGraph(
                        ...
                        formatSolve(R.string.verbal_rectangle_parallel_line_2,
                            (thisElement as Line).startNode.char.toString() +
                                    thisElement.finalNode.char,
                            figure.mLines[(i + 2) % 4].startNode.char.toString() +
                                    figure.mLines[(i + 2) % 4].finalNode.char),
                        formatSolve(R.string.expression_rectangle_parallel_line_2,
                            thisElement.startNode.char.toString() + thisElement.finalNode.char,
                            figure.mLines[(i + 2) % 4].startNode.char.toString() + figure.mLines[(i + 2) % 4].finalNode.char))
            }
                ...
                for(j in 1..3)
                    if (figure.mAngles[(i+j) % 4].getValue() == null)
                        figure.mAngles[(i+j) % 4].setValueGraph(
                            90f,
                            emptyList(),
                            formatSolve(R.string.verbal_rectangle_angle_90_1,
                                (thisElement as Angle).startNode.char.toString() +
                                        thisElement.angleNode.char +
                                        thisElement.finalNode.char),
                            formatSolve(R.string.expression_rectangle_angle_90_1,
                                thisElement.startNode.char.toString() +
                                        thisElement.angleNode.char +
                                        thisElement.finalNode.char))// fix
            }
        }
    }
}
